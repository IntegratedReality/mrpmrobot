...
int main(int argc, char **argv)
{
	...
	DriveClass drive(MotorClass(22, 27, false),
      ...
      receiver.getData(ID).pos.x,
      receiver.getData(ID).pos.y,
      receiver.getData(ID).pos.theta,
      receiver.getData(ID).time
      );
	...
	std::thread robot_control_thread([&](){
		while (true) {
			while (!receiver.checkMessageReceived());
			mutex_obj.lock();
			drive.updateData(receiver.getData(ID).pos.x,
          receiver.getData(ID).pos.y,
          receiver.getData(ID).pos.theta,
          receiver.getData(ID).time);
			drive.updateDrive();
			...
			if (ID < 3 
          && !(receiver.getData(ID).isAI ||
            receiver.getData(ID).state == DEAD ||
