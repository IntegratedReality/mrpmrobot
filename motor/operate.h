...
class Operate
{
    ...
    std::array<double, 2> max_velocity;
    std::array<double, 4> now_position;
public:
    Operate(DriveClass drive) : drive(drive)
    {}
    bool setTargetPosition(double target_x, double target_y, double target_theta, double target_v, double max_v, double max_omega);
